...
public class TickSeekBar extends View {
    ...
    private int[] mTickMarksColorArray;
    private boolean mCustomTickMarksColorResult;
...
//        initTickXAndTextXLocation();
    ...
    private void drawTickMarks(Canvas canvas) {
        ...
        for (int i = 0; i < mTickMarksX.length; i++) {
            ...
            if (mCustomTickMarksColorResult) {
                mStockPaint.setColor(mTickMarksColorArray[i]);
            }
            if (mTickMarksDrawable != null) {
            ...
            } else if (mShowTickMarksType == TickMarkType.V_LINE) {
                int w = SizeUtils.dp2px(mContext, 1f);
                int h = SizeUtils.dp2px(mContext, 4);
                canvas.drawRect(mTickMarksX[i] - w, mBackgroundTrack.top - h, mTickMarksX[i] + w, mBackgroundTrack.bottom + w, mStockPaint);
            }
        }
    }
    ...
    public void customTickMarksColor(@NonNull ColorCollector collector) {
        int[] colorArray = new int[mTicksCount];
        for (int i = 0; i < colorArray.length; i++) {
            //set the default section color
            colorArray[i] = mBackgroundTrackColor;
        }
        this.mCustomTickMarksColorResult = collector.collectColor(colorArray);
        this.mTickMarksColorArray = colorArray;
        invalidate();
    }
    ...
    public void customSectionTrackColor(@NonNull ColorCollector collector) {
        ...
        this.mCustomTrackSectionColorResult = collector.collectColor(colorArray);
